...
from ..exceptions import ArchyConfigError, ArchySecurityError, ArchyGitError
...
class ArchyConfig(BaseModel):
    ...
    project_path: Path = Field(
        default=Path("."),
        description="Path to the git project directory"
    )
    subfolder: Optional[str] = Field(
        default=None,
        description="Subfolder to focus analysis on"
    )
    arch_filename: str = Field(
        default="arch.md",
        description="Architecture document filename"
    )
    ...
    ai_backend: AIBackend = Field(
        default=AIBackend.CURSOR_AGENT,
    ...
    BLOCKED_SYSTEM_DIRS: List[str] = Field(
        default=["/etc", "/sys", "/proc", "/dev", "/boot", "/root"],
        exclude=True
    )
    ...
    def validate_project_path(cls, v: Path) -> Path:
        ...
        if '..' in path_str or path_str.startswith('/'):
            if any(blocked in path_str for blocked in ["/etc", "/sys", "/proc", "/dev", "/boot", "/root"]):
                raise ArchySecurityError(f"Access to system directory not allowed: {path_str}")
            
            # Allow relative paths with .. if they don't go to system dirs
            if re.search(r'\.\..*/', path_str):
                resolved = Path(path_str).resolve()
                if any(str(resolved).startswith(blocked) for blocked in ["/etc", "/sys", "/proc", "/dev", "/boot", "/root"]):
                    raise ArchySecurityError(f"Resolved path accesses system directory: {resolved}")
    ...
    def setup_paths_and_git_context(self) -> 'ArchyConfig':
        ...
        try:
            ...
            if not self.project_path_abs.exists():
                raise ArchyConfigError(f"Project path does not exist: {self.project_path}")
...
class ArchySettings(BaseSettings):
    ...
    ai_backend: AIBackend = Field(
        default=AIBackend.CURSOR_AGENT,
        description="Default AI backend to use"
    )
    max_file_size: int = Field(
        default=10_000_000,
        description="Maximum file size to analyze (10MB)"
    )
