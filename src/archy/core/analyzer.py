...
from ..backends.base import get_backend, clean_architecture_response
from ..exceptions import ArchyError, ArchyAIBackendError
...
class ArchitectureDocument:
    ...
    def __init__(self, content: str, file_path: Path):
        ...
        self.sections = {}  # Will be populated when we parse the content
...
class ArchitectureAnalyzer:
    ...
    def __init__(self, config: ArchyConfig, progress: Optional[Progress] = None):
        ...
        try:
            self.ai_backend = get_backend(config.ai_backend.value)
    ...
    def generate_fresh(self) -> ArchitectureDocument:
        ...
        prompt = pattern_manager.create_fresh_prompt(
            ...
            tracked_files=[str(f) for f in self.git_analysis.all_tracked_files],
        ...
        try:
            ...
            return ArchitectureDocument(
                ...
                file_path=self.config.arch_file_path
            )
            
        except ArchyAIBackendError as e:
            # If AI backend fails, save prompt for manual processing
            prompt_file = self.config.arch_file_path.parent / f"{self.config.arch_file_path.stem}_prompt.txt"
...
"""
    ...
    def _get_directory_structure(self) -> str:
        ...
        try:
            ...
            result = subprocess.run(
                ['tree', str(self.config.analysis_target_abs), '-I', 
                 'node_modules|.git|__pycache__|dist|build|target'],
                capture_output=True, text=True, timeout=10
            )
            return result.stdout if result.returncode == 0 else "Directory structure unavailable (tree command failed)"
    ...
    def _simple_directory_listing(self) -> str:
        ...
        try:
            ...
            if len(self.git_analysis.all_tracked_files) > 20:
                paths.append(f"  ... and {len(self.git_analysis.all_tracked_files) - 20} more files")
            return "\n".join(paths)
        except Exception:
    ...
    def _update_existing_architecture(self, changes_summary: str) -> ArchitectureDocument:
        """Update existing architecture file with changes using pattern template."""
        # Read existing content
        try:
            with open(self.config.arch_file_path, 'r', encoding='utf-8') as f:
                existing_content = f.read()
        except Exception as e:
            raise ArchyError(f"Failed to read existing architecture file: {e}")
        ...
        try:
            response = self.ai_backend.generate(prompt, force=True)  # Use force for updates
            ...
            # If AI backend fails, save prompt for manual processing
            prompt_file = self.config.arch_file_path.parent / f"{self.config.arch_file_path.stem}_update_prompt.txt"
            with open(prompt_file, 'w', encoding='utf-8') as f:
                f.write(prompt)
...
"""
    ...
    def _create_from_changes(self, changes_summary: str) -> ArchitectureDocument:
        ...
        changed_file_paths = [str(change.file_path) for change in self.git_analysis.changed_files]
...
"""
        ...
        try:
            ...
            # If AI backend fails, save prompt for manual processing
            prompt_file = self.config.arch_file_path.parent / f"{self.config.arch_file_path.stem}_create_from_changes_prompt.txt"
            with open(prompt_file, 'w', encoding='utf-8') as f:
