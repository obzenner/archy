    ...
    def generate(self, prompt: str, force: bool = False) -> AIResponse:
        ...
        try:
            ...
            if result.returncode != 0:
                error_msg = result.stderr.strip() if result.stderr else "Unknown cursor-agent error"
                raise ArchyAIBackendError(f"cursor-agent failed (exit code {result.returncode}): {error_msg}")
