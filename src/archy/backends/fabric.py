...
class FabricBackend(AIBackend):
    ...
    def generate(self, prompt: str, force: bool = False) -> AIResponse:
        ...
        try:
            ...
            result = self._run_command(cmd, input_text=prompt, timeout=self.config.timeout)
            ...
            if result.returncode != 0:
                ...
                if any(error in stderr_lower for error in ["error", "failed", "not found", "invalid"]):
                    error_msg = result.stderr.strip() if result.stderr else f"fabric-ai failed (exit code {result.returncode})"
